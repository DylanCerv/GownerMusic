...
if ($register->validateData($username, $email, $v_email, $password, $v_password) === TRUE || $username == NULL){
    echo "<script>alert('ERROR: Llene todos los campos porfavor');window.location='../../?cm=singup&m=register'</script>";
}else{
    //Comprobar si el usuario ya se encuentra registrado
    if ($register->validateUsernameExist_DB($username) === TRUE) {
        echo "<script>alert('ERROR: Usuario ya registrado');window.location='../../?cm=singup&m=register'</script>";
    }else{
        ...
        if ($register->validateEmailRigth($email, $v_email) !== TRUE){
            ...
            if ($register->validateSameEmail($email, $v_email) !== TRUE){
                echo "<script>alert('ERROR: Emails diferentes, porfavor ingrese los mismos emails para poder verificarlos');window.location='../../?cm=singup&m=register'</script>";
            }else{
                //Comprobar si el email ya se encuentra registrado
                if ($register->validateEmailExist_DB($email) === TRUE){
                    echo "<script>alert('ERROR: El email ya se encuentra registrado');window.location='../../?cm=singup&m=register'</script>";
                }else{
                    //Comprobar si las contraseñas son iguales
                    if ($register->validateSamePassword($password, $v_password) !== TRUE){
                        echo "<script>alert('ERROR: Contraseñas diferentes, porfavor ingrese las mismas contraseñas para poder verificarlas');window.location='../../?cm=singup&m=register'</script>";
                    }else{
